...
public class FileUtil {
   ...
   * @param file
   */
  public static void cleanFolder(File file, String exclude) {
    if (!file.isDirectory())
      return;

    File[] _files = file.listFiles();
    for (File _f : _files) {
      if (_f.getName().endsWith(exclude))
        continue;
      delete(_f);
    }
  }
  ...
  public static String getCmaqFile(int year, int days, String dataPath) {
    return dataPath + String.format("Combine.Surf.Column.AOD.together.%d%03d", year, days);
  }
